...
\lettrineabstract{We implement various modern FOC techniques on commercial brushless DC motors. We integrate the work of several authors in on a STM32F103 microcontroller using standard C++. TODO add testing results and physical implementation when it's done. Also add performance considerations regarding FPU stuff.}
...
For simplicity's sake, we define saliency as the ratio between the impedance of the direct axis and the quadrature axis ($k_s=\frac{Z_d}{Z_q}$). This allows us to simplify equation \ref{iq_wave} into equation.\ref{iq_wave2}.

\begin{equation}
  \hat{i}_q = -\frac{-(1 - k_s)V_h}{2Z_d}\cos(\omega_h t)\sin(2\Delta\theta)
  \label{iq_wave2}
\end{equation}

Notably however, our signal is still modulated by our injection wave. This is where \cite{SUN20226457} and \cite{dmytrohfi} diverge. \cite{SUN20226457} uses a low-pass filter to extract the "DC" signal, removing our carrier frequency. \cite{dmytrohfi} uses a superheterdyne demodulator (fig. \ref{heterodyne}) to extract the signal.
\begin{figure}
  \begin{circuitikz}[american]
      \draw (0,0) node[anchor=south] {$\hat{i}_q$}
      to[short, o-] ++(1,0)
      to[bandpass,>,l=$f_h$] ++(1,0)
      to[short] ++(1,0) node[mixer,anchor=west](m){}
      (m.w) node[inputarrow]{}
      (m.s) node[inputarrow,rotate=90]{} -- ++(0,-0.5) node[oscillator,anchor=north, label={[anchor=north]below:$f_h$}]{};
      \draw (m.e) to[bandstop,>,l=$f_h$] ++(3,0)
      to[short, -o] ++(0,0) node[anchor=south]{$\propto\sin(2\Delta\theta)$};
  \end{circuitikz}
  \caption{Superheterodyning extracts error signal from high frequency carrier signal}
  \label{heterodyne}
\end{figure}

\section{Filter Comparison}

As a part of implementation, we compare the control loop described by \cite{SUN20226457} utilizing a low-pass filter and a control loop described by \cite{dmytrohfi} utilizing a superheterdyne filter. We model using a rotor speed of 100 radians per second, beyond which we assume back EMF generation is significant enough to be measured for position sensing. We include both a sinusoidal injection waveform and a pulse injection waveform in our analysis. Notably, since our hardware does not include output filtering. For this reason, a square wave was used as the injection waveform.  Regardless, the sinusoidal waveform is included in analysis for parity with the other papers.


\subsection{Results}

The algorithms of \cite{scipy} are used in simulation. For the lowpass filter approach in \cite{SUN20226457}, a 4-pole FIR filter was used. The 4-pole FIR filter was used to simulate a relatively low-computation filter that would be used in the firmware, as floating point operations are slow due to the lack of a FPU. For the heterodyning approach by \cite{dmytrohfi}, biquad filters were used for the bandpass and the bandstop filters. Biquad IIR filters were used to trade off between performance and speed. Additionally, a undersampling implementation was tested but was considered impractical. Constant polling of the ADC was necessary for FOC anyways, and the bandpass filter would have to be implemented in hardware to see performance savings.

\begin{figure}
  \includegraphics*[width=8cm]{simplots/sinusoidal_injection.jpg}
  \caption{Sinusoidal Injection}
  \label{sin_hfi}
\end{figure}

\subsubsection{Sinusoidal}

Starting with our base case, the amplitude modulation of the rotor error signal with the pulse wave creates two lobes apart from the carrier frequency. (Figure \ref{sin_hfi}) The lowpass filter imployed by \cite{SUN20226457} attenuates this signal, but does not meaningfully seperate the carrier frequency from the signal. Superheterodyning manages to seperate the signals, at the cost of attenuation. Notably, undersampling without employin another bandpass filter results in unwanted harmonics.

\subsubsection{Square Wave}

\begin{figure}
  \includegraphics*[width=8cm]{simplots/square_wave_injection.jpg}
  \caption{50\% Pulse Wave Injection}
  \label{sq_hfi}
\end{figure}

\begin{figure}
  \includegraphics*[width=8cm]{simplots/10p_pulse_injection.jpg}
  \caption{10\% Pulse Wave Injection}
  \label{pulse_hfi}
\end{figure}
u
Square waves, when compared to sine waves, contain lots of harmonics. This is noticable in the large tail of high frequencies in both figure \ref{sq_hfi} and \ref{pulse_hfi}. The superheterodyning filter however does not pick up any harmonics and filters them out. In figure \ref{sq_hfi}, the amplitude of the fundamental frequency is the same as that of the sine wave, so the end result is nearly identical to the sine wave case.
However, in figure \ref{pulse_hfi}, the amplitude of the fundamental frequency is significantly smaller. This results in a significantly smaller signal for the superheterodyne method. Moreover, due to the waveform approaching DC behavior, the original signal is not entirely attenuated out. Due to that, the lowpass filter design manages to outperform the superheterodyne implementation by magnitudes.
